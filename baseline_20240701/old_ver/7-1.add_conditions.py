    ...
    for order_id in range(K):
        for order1, order2 in itertools.combinations(merge_possible[rider_of_choice][order_id], 2):
            ...
            for rider_type, rider_inst in rider.items():
                ...
                cost = np.inf
                tmp_time = tmp_dist/rider_inst.speed

                if tmp_time <= (max_deadline-min_readytime) and tmp_min_shop_dlv/rider_inst.speed <= (min_deadline-max_readytime):
...
def can_merge(K, all_orders, bundle_index, rider):
    shops = defaultdict(lambda : np.inf)
    dlvs = defaultdict(lambda : 0)
