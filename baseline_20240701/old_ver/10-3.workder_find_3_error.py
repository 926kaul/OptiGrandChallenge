...
def worker(task_queue, result_queue, K, all_orders, all_riders, dist_mat, start_time, timelimit):
    ...
    while True:
        ...
        for task_tuple in task:
            ...
            # 3 order bundle은 한결이가 만든 조건식 추가
            if len(task_tuple) == 3:
                ...
                print(time.time()-start_time, 'task done', task_count)
            else:
                for rider_type, rider_inst in rider.items():
                    optimal_route_found = None
                    cost = np.inf
...
# 메인 함수 p0
def algorithm(K, all_orders, all_riders, dist_mat, timelimit=59):
    ...
    # 모든 order에 대해 가능한 조합을 찾아서 order_numbers개를 묶어서 task_queue에 넣어줌
    # 묶지 않으면 성능저하가 일어남
    considered = set()
    rider_of_choice = "ALL"
