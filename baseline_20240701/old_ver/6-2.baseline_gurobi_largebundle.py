...
def large_bundle(q, K:int, all_orders:list[Order], all_riders, merge_result, dist_mat, sorted_index, started_time, timelimit):
    def get_large_bundle(q, K:int, all_orders:list[Order], rider:Rider, merge_result, dist_mat, size_bundle, sorted_index, blacklist):
            ...
            for shop_perm in permutations(tmp_seq):
                for dlv_perm in permutations(tmp_seq):
                    if test_route_feasibility(all_orders, rider, shop_perm, dlv_perm) == 0:
                        ...
                        q.put(merge_result)
                        return get_large_bundle(q, K, all_orders, rider, merge_result, dist_mat, size_bundle, sorted_index, blacklist)
            return
        return
    
    size_bundle = 0
    for item in merge_result:
        size_bundle = max(size_bundle,len(item[0]))
    while True:
        if time.time() - started_time > timelimit:
            return merge_result
        for rider in all_riders:
            get_large_bundle(q, K,all_orders,rider,merge_result,dist_mat,size_bundle,sorted_index,[])
        size_bundle += 1
...
def algorithm(K, all_orders, all_riders, dist_mat, timelimit=58):
    ...
    while True:
        ...
        # 다른 프로세스의 결과 가져오기
        old_merge_result_len = len(merge_result)
        if not q.empty():
            if no_change_merge_result < 3:
                merge_result = q.get()

                if old_merge_result_len == len(merge_result):
                    no_change_merge_result += 1
                else:
                    no_change_merge_result = 0
                ...
                if no_change_merge_result == 3:
                    ...
                    p = Process(target=large_bundle, args=(q, K, all_orders, all_riders, merge_result, dist_mat, sorted_index, start_time, timelimit-1))
                    p.start()
                    no_change_merge_result += 1
                else:
                    merge_result = q.get()
